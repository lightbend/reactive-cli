      ...
      .flatMap {
        case (key, value) =>
          if (key.startsWith(s"$namespace.")) {
            val keyPart = key.drop(namespace.length + 1)
            val numeric = keyPart.takeWhile(_ != '.')
            val regular = keyPart.dropWhile(_ != '.').dropWhile(_ == '.')

            if (numeric.forall(_.isDigit))
              Some((numeric.toInt, regular, value))
            else
              None
          } else {
            None
